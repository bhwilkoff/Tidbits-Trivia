...
| Rolling named-scenario health regression | `roll1-*` re-runs on the current build | ✅ NINE consecutive full laps green (roll2–roll10, ~11 hours of continuous rotation) — all 12 named scenarios each lap; the CHROME MULTIPLAYER LOOP has passed TWICE as the lap-closer (real browser joins, same question both screens, answer counted, reveal + points back in Chrome; passes #1 and #2 on 2026-08-25) |
...
- **F-006** (2026-08-25) — OPEN, low severity, watch-item. *A night restarted
  on the same room code inherits prior answers for matching question ids*:
  Chrome-loop pass #2's freshly started night showed "1 answered" on Q1
  before the new answer arrived — the previous session's answer under the
  same `live/QATV/answers/{qid}` persisted. Player impact is limited
  (production room codes are random per night; this needs code reuse, e.g.
  a host crash-recovery restart). Candidate fix: namespace answer paths by
  a per-night session id, or clear `answers/` when a host (re)opens a room.
  Not blocking; the pinned-code QA loop tolerates it.
