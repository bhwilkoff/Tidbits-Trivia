...
| Rolling named-scenario health regression | `roll1-*` re-runs on the current build | ✅ SIX consecutive full laps green (roll2–roll7, ~7.5 hours of continuous rotation spanning midnight) — all 12 named scenarios each lap; the single 'failure' across all laps was a harness regex gap on a healthy cross-midnight daily, fixed + re-verified (`roll2..7-*` dirs) |
