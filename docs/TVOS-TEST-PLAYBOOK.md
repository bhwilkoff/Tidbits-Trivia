...
| Rolling named-scenario health regression | `roll1-*` re-runs on the current build | ✅ THREE consecutive full laps green (roll2, roll3 across midnight, roll4 on 2026-08-25 incl. the fresh daily Link Wall board) — all 12 named scenarios each lap; the single roll4 'failure' was a harness regex gap on a healthy cross-midnight daily, fixed + re-verified (`roll2-4-*` dirs) |
