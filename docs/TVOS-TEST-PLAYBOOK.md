...
| Rolling named-scenario health regression | `roll1-*` re-runs on the current build | ✅ THIRTEEN consecutive full laps green (roll2–roll14, ~15+ hours of continuous rotation) — all 12 named scenarios each lap; the CHROME MULTIPLAYER LOOP has passed SIX times as the lap-closer (real browser joins, same question both screens, answer counted, reveal + points back in Chrome; passes #3–#6 also asserted the F-006 invariant: a fresh room reads "0 answered" before any live answer) |
