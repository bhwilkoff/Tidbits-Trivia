...
- [~] 2.24 Async friend duels — `DuelStore` (Core) COMPLETE: wire types byte-keyed to js/duels.js (DuelQuestion p/o/c/e, DuelPlayer, Duel, DuelInvite), pure logic (Compact/QuestionsOf round-trip, Classify outcome state machine YourTurn/WaitingOnThem/Won/Lost/Tie, newest-first capped local id tracking), and the networked methods over FirebaseRtdb (Challenge+invite, Submit own slot, Inbox, Accept, Mine). 8 offline tests. Serverless duels/{id}+duelInbox contract. Presentation layer DONE too: DuelsUi.BuildPanel renders inbox + my-duels with color-coded outcome chips (Your turn/coral·Play, Waiting/gray, Won/green, Lost/red), PNG-verified; DuelStore wired into GameData. Remaining: a shared authed FirebaseRtdb in the App + the challenge-a-friend + play-frozen-set entry points (networked, 2-device gated)
