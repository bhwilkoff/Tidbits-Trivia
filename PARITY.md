...
| Tidbits Live — Wave A run-of-show depth (2026-07-04) | ✅ join | ✅ join | ⏳ join | ✅ join | **Wave A COMPLETE (host authoring & run-of-show depth), 1.6.27.** macOS HOST (builder + cockpit): "story behind the answer" on reveal, difficulty/category **balance meter**, per-round **timers**, **drag-to-reorder**, **CSV import**, per-round **host notes**, and a **final wager round** (mark a round Wager → teams stake 0…their score → correct +stake / wrong −stake on reveal, clamped to score). JOIN-CLIENT display fans to **web/iOS/Android**: story on the reveal screen, ticking countdown (coral ≤5s), wager stake input (iOS Stepper / web+Android slider). Big screen (macOS) shows story + countdown. **tvOS-as-join ⏳** — the Wave A join-display isn't in `TVLivePlayerView` (joining a pub quiz from a TV is an edge case; host is Mac, players are phones). Wire: `Pub.story/deadline/wager` + `Answer.wager` + `LiveRound.{timerSeconds,hostNote,isWager}` (all optional/additive). All 5 build green |
