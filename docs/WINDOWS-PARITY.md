...
- [x] 3.39 Question editor: Picture section (Choose picture… into the store, preview, remove, or URL) + a Clip row (Choose audio… / Choose video… / Remove) that attaches to THIS question via `ClipAt`/`SetClip` keeping `_clips` index-parallel (LIVE-PACKAGE-FORMAT §8.1, 2026-09-07). **Remaining [~]:** drag-and-drop a file onto a question row (the Mac has it; Avalonia 12's DataTransfer API needs a windows-latest check)
