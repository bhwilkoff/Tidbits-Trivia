...
| Reveal explanations on the glass (the learn-something promise) | `tools/atv_reveal_audit.py` — 6 date-seeded rows to REVEAL | ✅ 2026-08-24: 6/6 explanation tails (up to 208ch) OCR-verified on the device reveal (`reveal-audit-1787624316`) |
| Random-picture on-device render (AsyncImage path, arbitrary rows) | `atv_run --luma` over 8 date-seeded random rows | ✅ 2026-08-24: 8/8 real photos on the glass (luma-gated) on the pruned build + picture-round regression green (`randpic-1..8`, `picture-round-postprune-*`) |
