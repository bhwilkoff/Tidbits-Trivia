...
| Rolling named-scenario health regression | `roll1-*` re-runs on the current build | ✅ FIVE consecutive full laps green (roll2–roll6, ~6 hours of continuous rotation spanning midnight) — all 12 named scenarios each lap; the single 'failure' across all laps was a harness regex gap on a healthy cross-midnight daily, fixed + re-verified (`roll2..6-*` dirs) |
