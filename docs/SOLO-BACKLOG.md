...
| Q3 | **Odd-one-out** | "Which doesn't belong?" — 3 countries from one continent + 1 outlier (`oddoneout.json`, 67 Qs from the corpus's `wd:continent` data). Plain 4-option MCQ (outlier = answer); zero new UI. No P31/P106 pass needed. | Yes (wd:continent) | P2 | ✅ all 4 platforms |
...
| F4 | **Answer-distribution telemetry** | Privacy-respecting per-option counts, **local-first foundation shipped** on all 4 platforms (`tidbits.answerTelemetry` keyed by question id; `RecordsStore.recordTelemetry` / `Store.recordTelemetry`; synthetic-chosenIndex modes skipped; no UI, no network). The **Predict the Crowd** reveal + "X% picked this" still 🔮 — needs a backend to aggregate the local counts across players (deferred to Ben). | P2 | ✅ foundation (4 platforms) · 🔮 crowd reveal (needs backend) |
