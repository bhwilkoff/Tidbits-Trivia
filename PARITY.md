...
|---|---|---|---|---|---|
| **Trivia Night** (configurable "bar trivia" night) | ⏳ | ✅ | ✅ | ⏳ | The new flagship default mode: a host-configured night of themed **rounds**, each round drawing one question TYPE — so one night pulls from EVERY question type (general MCQ → picture → which-first → closest → ordering → matching → name-it → odd-one-out → name-as-many). Presets (Quick / Pub / The Works) + per-round counts + category. A client meta-mode over the shape-routing engine (`NightPlan` + `GameEngine.startNight`, no corpus change — Decisions 025/031). Round banners + end-of-round beats + the shared missed-fact recap. iOS: home card → `NightSetupView` config → `NightContainerView`. tvOS: home hero → ten-foot `NightSetupView_tvOS` (preset+category) → `TVNightContainer`. **Solo + pass-and-play + TV-host buzzer are phased** (see §3b living-room rows). web/Android ⏳ next slice |
...
| Buzz Night (same-room buzz game) | 🔮 | 🚧 controller | 🚧 host | 🔮 | TV is stage + scoreboard, phones buzz, **first correct buzz scores**, a wrong buzz locks that seat out and re-opens to others, every Q ends on the Learn-the-fact reveal (GAME-MODES-RESEARCH D2). Buzzable MCQ rounds only (a phone can't drive a slider/board). tvOS `BuzzNightView_tvOS` (lobby → buzz loop → standings) + iOS `BuzzerJoinView`. Built + sim-verified to the host UI; the live buzz handshake needs a 2-device hardware test (Ben). Web/Android via Phase 2 |
