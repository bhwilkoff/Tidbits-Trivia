...
| Rolling named-scenario health regression | `roll1-*` re-runs on the current build | ✅ FOUR consecutive full laps green (roll2–roll5, spanning midnight into 2026-08-25, incl. the fresh daily Link Wall board and the new day's daily set) — all 12 named scenarios each lap; the single 'failure' across all laps was a harness regex gap on a healthy cross-midnight daily, fixed + re-verified (`roll2..5-*` dirs) |
