...
- [~] 3.52 Social graph — `FriendStore` (Core, JSON, dedup-by-uid, private/local mirroring the owner-only bucket) + a "Friends" section leading the Leaderboard (your added people ranked by their public overall standing, "—" if not yet on the board, no champion chip). Co-players are already captured at night-end (LivePlayerClient.Coplayers); the one-tap "add the people you played with" UI at the join wrap is the remaining bit. Store + friends-section tests + PNG
