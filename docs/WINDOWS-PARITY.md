...
- [x] 3.83 The round scoreboard shows movement, and opens by itself (macOS-DESIGN A2.15, 2026-09-10) — `LiveStandingsMove` (Core, pure: `Moves`/`Label`/`BiggestClimb`, byte-identical to Swift's, 6 xUnit), `LiveNightHost.OpenRoundScoreboard()`/`ScoreboardMoves`/`ScoreboardEpoch` called from `Next()` itself when it crosses a round boundary, and the projector draws a ▲/▼/NEW chip per row plus the biggest-climb line. **Found on the glass:** the first cut opened the scoreboard from the view model's `Next()`, so a host driving the night from the PHONE REMOTE (which calls `Host.Next()` directly) got no scoreboard at all — the decision moved into Core, where every path goes through it. **Verified on the real box** (`scratchpad/e2e_movement.py windows`): a three-round night driven entirely by the remote, Table B overtaking Table A in round 2; the projector read "SCORES AFTER ROUND 2 · Biggest climb: Table B up 1" with ▲1 / ▼1.
- [x] 3.82 The Windows JOINER plays the joker (A2.14, 2026-09-10) — `LivePlayerClient.JokerRound`/`PlayJoker` (read back on join), a ComboBox of the rounds still ahead above every answer shape in `JoinPlayerView`, and a quiet "played on Round N" line once the round has begun. Hook `TIDBITS_LIVE_JOKER=<round>`. **Verified on the real box:** joined a Mac-hosted night, picked round 2, and the Mac cockpit badged "Windows Table JOKER · R2".
- [x] 3.81 The joker (macOS-DESIGN A2.14, 2026-09-10) — `LiveJoker` (Core, pure: `Playable`/`Played`/`Multiplier`/`Line`, byte-identical to Swift's), `LiveRoom.JokerRound`/`Joker` + `Pub.JokerRounds`, `LiveEvent.Joker` (+ the file contract's `joker`), the builder's "Teams play a joker" checkbox, `LiveHostNet` streams `jokers/{uid}` (cleared on a fresh open), `LiveNightHost` locks a round's picks in `PublishCurrent()` when its first question goes out (`JokersPlayed`, in the resume snapshot), doubles points AND the penalty in `ScoreAnswers`, doubles a paper table's ± in its joker round (`SetPaperJoker`, the row's Joker button cycles the rounds ahead), badges every row (`Joined.Joker`), and the projector gains a switchable "jokers" line. 6 xUnit.
- [x] 3.80 A night the app died in the middle of can be picked back up (macOS-DESIGN A2.13, 2026-09-10) — `NightResume` + `ResumedNight` (Core; JSON at `%LOCALAPPDATA%/TidbitsTrivia/live-resume.json`, six-hour shelf life), written by `LiveNightHost.SaveResume()` on the host's deliberate moves (next, reveal, a paper team, a hidden name — not per stream event) and cleared in `End()`. The Live landing shows "A night is still going — <title>, question N of M, room CODE" with Pick it back up / Discard; resuming sets `LiveNightHost.Resuming`, so `Open()` re-opens the SAME code with `resuming: true` (PATCHes meta so `meta/names` survives; keeps `scores/` instead of the fresh-night wipe) and `Start()` restores the index, reveal state, points, paper teams with their scores, hidden uids and the answer sheet. Hook `TIDBITS_LIVE_RESUME=1` (Diag `resume hook: pending=CODE @N`) — a crash cannot be staged by hand. 7 xUnit. **Verified on the real box** (`scratchpad/e2e_resume.py windows`): a night hosted to q2 with a wire table scored, the process force-killed, relaunched with the hook — room 359V re-opened on q2, `scores/` still held the point, both teams still in, the cockpit photographed with the standings; the night then ended on the finish hook and `live-resume.json` was gone.
- [x] 3.79 "N of M answered" on the big screen (macOS-DESIGN A8.11, 2026-09-10) — a twelfth switchable projector element (`answersIn`), the shared pure `LiveProgress.AnswersIn` (3 xUnit, byte-identical to Swift's), shown while a question is live and hidden before anyone joins. **Verified on the real box:** two wire tables joined, one answered, and the projector read "1 of 2 answered".
- [x] 3.78 Countdowns speak in the HOST's clock (2026-09-10) — found while photographing the break: this box runs **101 s behind the Mac**, so it showed a 10-minute break as 12, and `Pub.Deadline` (the per-question timer, shipped since Wave A) was wrong by the same amount. `LiveClock` (Core, pure, 5 xUnit incl. the bench symptom), `Pub.Now` stamped by every host, `LivePlayerClient.HostOffsetMs` kept per pub, and `SecondsRemaining`/`BreakHeadline`/`BreakSubline` evaluated in the host's frame. Additive — a pub without `now` yields offset 0 and the old behaviour. **Verified on the real box:** clock still 101 s out, break headline now identical to the web's.
- [x] 3.77 The break (macOS-DESIGN A3.14, 2026-09-10) — Windows had NO hold at all: the Mac could park the show on a break slide since Wave A and the Windows cockpit could only keep asking questions. `LiveNightHost.OnBreak`/`BreakUntil` + `SetBreak(on, minutes)` (republishes, so the phones learn at once), `LiveBreak` (pure, 5 xUnit, byte-identical to Swift's), `Pub.OnBreak`/`BreakUntil`, a projector break slide over every other screen, two cockpit commands (**Hold — the room is on a break** and **Break with a return time…**, distinct from the existing standings hold), and the Windows JOINER shows the break instead of the question (`ShowQuestionOnly`/`ShowOptions` yield to it, so nobody answers during the interval). Hook `TIDBITS_LIVE_BREAK=<minutes>` + `_AT` (Diag `break hook: onBreak=… headline=…`). **Verified on the real box** as a joiner of a Mac-called break.
- [x] 3.76 The night is kept (macOS-DESIGN A2.12, 2026-09-10) — `NightArchive` + `ArchivedNight` (Core; JSON at `%LOCALAPPDATA%/TidbitsTrivia/live-nights.json`, newest first, Cap 20 / LogCap 600, the A3.11 report recomputed from the archived sheet rather than stored), recorded once in `LiveNightHost.End()` (the one place the last `Next()` and the host's own finish converge; `LiveHostNet.Venue` is kept so the night files under its room). The Live landing gains a "Nights you've run" list under the saved events — date · headline · venue, **Open** (a dialog: standings, how the night went, hardest/easiest) and forget. Export answer sheet re-runs `LiveExport.AnswersCsv` from the archived log. Hooks `TIDBITS_LIVE_NIGHTS=1` (open the newest night; Diag `nights hook: archived=N`) and `TIDBITS_LIVE_FINISH_AT=<secs>` — reveal then END the night, the macOS twin, without which a two-question night on the box could never be driven to its wrap. 6 xUnit. **Verified on the real box** (`scratchpad/e2e_nights.py windows`): a night hosted to its end, the app QUIT, then relaunched — the archive held it with Table 1 at 1, Table 2 at 0 and the report reading 50% answers right · 100% tables answering.
