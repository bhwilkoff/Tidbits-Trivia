...
| Rolling named-scenario health regression | `roll1-*` re-runs on the current build | ✅ TEN consecutive full laps green (roll2–roll11, ~12+ hours of continuous rotation) — all 12 named scenarios each lap; the CHROME MULTIPLAYER LOOP has passed THREE times as the lap-closer (real browser joins, same question both screens, answer counted, reveal + points back in Chrome; pass #3 doubled as the F-006 fix verification: fresh room read "0 answered" with a stale ledger row present) |
