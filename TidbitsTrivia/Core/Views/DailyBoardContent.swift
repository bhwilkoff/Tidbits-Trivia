...
/// The Daily's global board, rendered (docs/DAILY-BOARD-CONTRACT.md). Shared across iOS,
/// macOS, and tvOS — pure SwiftUI, no platform chrome, so each platform supplies its own
/// presentation wrapper (sheet + Done). Reads the static JSON the hourly cron publishes
/// (free/cacheable, never RTDB); computes the percentile locally from the histogram.
struct DailyBoardContent: View {
    let day: String
    let myScore: Int
    ...
    private var scoreCard: some View {
        VStack(spacing: 8) {
            ...
            Text("\(myScore)").font(.system(size: 56, weight: .black, design: .rounded)).foregroundStyle(Tidbits.Palette.ink)
    ...
    @ViewBuilder private func standing(_ board: DailyBoard.Board) -> some View {
        if let pct = DailyBoard.percentile(hist: board.hist, myScore: myScore) {
            VStack(spacing: 2) {
                Text("\(pct)%").font(.system(size: 44, weight: .black, design: .rounded)).foregroundStyle(Tidbits.Palette.ink)
