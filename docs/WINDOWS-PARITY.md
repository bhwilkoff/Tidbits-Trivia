...
- [~] 2.15 Topic Levels (per-domain XP bars) + **Badges** (levelable milestones via BadgeMath — earned-only, coral tier-number icon per R-ICON-1) + **Stake calibration** readout (per-confidence-tier hit rate — Sure/Likely/Hunch, bar + "hits/total · %", from the already-tracked RecordsStore.Calibration; play-through test + PNG) all shipped in the Records dashboard. Pie + avatar re-roll pending; liveNights=0 until live-night records are tracked locally
...
- [~] 2.22 Spaced re-asking — due missed facts (RecordsStore.DueReview) are now woven into MCQ games via engine.Start(review:), opt-out via the Settings review toggle; skips Daily + non-MCQ shapes. Resolves on a correct answer (RecordsStore.Record). End-to-end test GREEN (miss → rewoven → resolved). Day-streak surfacing at the results moment pending
