...
| Rolling named-scenario health regression | `roll1-*` re-runs on the current build | ✅ ELEVEN consecutive full laps green (roll2–roll12, ~13+ hours of continuous rotation) — all 12 named scenarios each lap; the CHROME MULTIPLAYER LOOP has passed FOUR times as the lap-closer (real browser joins, same question both screens, answer counted, reveal + points back in Chrome; passes #3 and #4 also asserted the F-006 invariant: a fresh room reads "0 answered" before any live answer) |
