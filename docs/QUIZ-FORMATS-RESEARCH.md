...
| Kahoot spreadsheet template | ✅ both hosts' CSV import (2026-09-07) | ✅ both hosts write real `.xlsx` (2026-09-07) | none (theirs carries none) | Import: instruction rows above the header are skipped, `1,3` keeps the first. Export: a minimal valid OOXML workbook in THEIR layout (instructions rows 1-6, header row 8, data from row 9, row number in A) because their importer takes only xlsx. Their caps (95 / 60 characters) and their allowed times are applied and REPORTED; a question that is not multiple-choice has no form in their sheet and is named rather than mangled. Validated by an independent reader (openpyxl) from both stacks |
...
| Open Trivia DB JSON | 🚫 **not built, deliberately** | — | none | A round could be seeded from their free MCQ bank, but Tidbits already ships 128k curated questions behind "Pull one from the corpus", and OpenTDB is user-contributed and uneven. Wiring it would duplicate an existing affordance while LOWERING the quality bar the corpus is held to (`quality-over-volume-steer`). Revisit only if a host asks for a topic the corpus genuinely cannot cover |
