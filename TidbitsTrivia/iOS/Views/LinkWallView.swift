...
/// Link Wall (Club Feature 6, Stage 2 — docs/CLUB-FEATURES-BUILD.md): the
/// NYT-Connections-style second daily, built on the content-clean generator
/// in `Core/Store/LinkWall.swift`. This is the iOS play surface only —
/// STAGED (iOS-reference-first) pending an owner call on the 5-platform
/// fan-out; the free Daily Tidbit is untouched.
///
/// Unlike `GameContainerView`, Link Wall never routes through `GameEngine` /
/// `GameMode` — it isn't a corpus MCQ round, so it owns its play loop here.
struct LinkWallView: View {
    let day: String
    var onDone: () -> Void
    ...
    var body: some View {
        NavigationStack {
            Group {
                if let puzzle {
                    if result.completed {
                        LinkWallResultView(day: day, puzzle: puzzle, result: result, onDone: onDone)
                    } else {
                        board(puzzle)
                    }
                } else {
                    ContentUnavailableView {
                        Label("Link Wall isn't ready", systemImage: "square.grid.3x3.fill")
                    } description: {
    ...
    /// in production. Waits for `loadIfNeeded()` to settle, then submits
    /// guesses through the exact same `submit()` path a real tap would.
    ...
    private func solvedRow(_ g: LinkWall.LinkWallGroup) -> some View {
        VStack(alignment: .leading, spacing: 3) {
            Text(g.label.uppercased())
                .font(Tidbits.TypeRamp.l3)
                .foregroundStyle(LinkWallPalette.color(for: g.difficulty).legibleForeground)
            Text(g.members.joined(separator: " · "))
                .font(Tidbits.TypeRamp.l5)
                .foregroundStyle(LinkWallPalette.color(for: g.difficulty).legibleForeground.opacity(0.85))
        }
        .frame(maxWidth: .infinity, alignment: .leading)
        .padding(14)
        .background(RoundedRectangle(cornerRadius: 14, style: .continuous).fill(LinkWallPalette.color(for: g.difficulty)))
    ...
    private var tileGrid: some View {
        LazyVGrid(columns: gridColumns, spacing: 8) {
            ForEach(remainingTiles, id: \.self) { tile in
                LinkWallTileButton(text: tile, isSelected: selected.contains(tile)) { toggle(tile) }
            }
        }
        .modifier(LinkWallShakeEffect(animatableData: shakeAmount))
    }
...
// MARK: - Result screen (win or loss — reveal + shareable emoji grid)

private struct LinkWallResultView: View {
    let day: String
                ...
                HStack(spacing: 4) {
                    ForEach(Array(row.enumerated()), id: \.offset) { _, difficulty in
                        RoundedRectangle(cornerRadius: 4)
                            .fill(LinkWallPalette.color(for: difficulty))
        ...
        VStack(spacing: 10) {
            ForEach(puzzle.groups, id: \.label) { g in
                VStack(alignment: .leading, spacing: 3) {
                    ...
                    Text(g.why)
                        .font(Tidbits.TypeRamp.l5)
                        .foregroundStyle(LinkWallPalette.color(for: g.difficulty).legibleForeground.opacity(0.9))
                }
                .frame(maxWidth: .infinity, alignment: .leading)
                .padding(14)
                .background(RoundedRectangle(cornerRadius: 14, style: .continuous).fill(LinkWallPalette.color(for: g.difficulty)))
    ...
    private var shareText: String {
        ...
        lines.append(contentsOf: result.guessHistory.map { row in row.map(LinkWallPalette.emoji(for:)).joined() })
...
private struct LinkWallTileButton: View {
    let text: String
    let isSelected: Bool
    let action: () -> Void
        ...
        Button(action: action) {
            Text(text)
                ...
                .background(RoundedRectangle(cornerRadius: 12, style: .continuous)
                    .fill(isSelected ? Tidbits.Palette.ink : Tidbits.Palette.surface))
...
private struct LinkWallShakeEffect: GeometryEffect {
