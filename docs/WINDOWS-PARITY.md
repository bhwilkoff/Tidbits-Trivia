...
- [ ] 3.71 Settings "Display name" → the portable profile's `name` (today it edits only the local `PlayerIdentityStore`; the Mac renames `players/{id}.name`). Merge the two stores or make the local one a cache of the portable profile.
