...
| Daily Six — global daily competition (Club, 2026-07-19) | 🚧 | ⏳ | ⏳ | ⏳ | **Building — first global-multiplayer feature (MONETIZATION §4c, `docs/DAILY-SIX-CONTRACT.md`).** Everyone worldwide plays the SAME six (`pickDaily(...,6)`, byte-identical incl. a NEW Python cron picker proven in `tools/daily-parity/run.sh`); one write to `dailySix/{day}/{uid}`; the hourly cron (`tools/aggregate_dailysix.py` + `dailysix.yml`) ranks + publishes `data/dailysix/{day}.json` (board + score histogram + per-Q global accuracy); clients read the static JSON, never RTDB (R-NET-1). **Playing + rank is FREE (R-MON-4); Club buys the autopsy.** DONE this pass: contract, RTDB rules, aggregator (sim-verified), cron, parity gate, web data layer (`DailySix` in api.js + `submitDailySix` in firebase.js), producer↔consumer percentile verified. REMAINING: web game-loop wiring + results view, then iOS/Android/tvOS/macOS/Windows ports (mechanical per the contract). **Rules need a `firebase deploy` (owner-gated).** Windows column tracked in `docs/WINDOWS-PARITY.md` |
