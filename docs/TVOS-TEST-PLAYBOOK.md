...
| Rolling named-scenario health regression | `roll1-*` re-runs on the current build | ✅ TWO consecutive full laps green (roll2 late 2026-08-24, roll3 spanning midnight into 2026-08-25) — all 12 named scenarios each lap, zero failures; the maintenance loop continues a lap every ~hour at the 5-min cadence (`roll2-*`, `roll3-*` dirs) |
