...
- [ ] 3.37 `/live/<code>` deep link — the projector QR / a pasted link should open the join screen with the code (R-JOIN-1, DEEP_LINKS.md). The JOIN A GAME button is now second on Play (build-verified on the Mac head; photograph on windows-latest)
- [ ] 3.36 Question pictures on the HOST surfaces — a Kahoot-imported night (LIVE-EVENT-FILE §7) carries `imageURL` on ordinary classic questions; the Mac big screen and cockpit now render it (2026-09-06). Windows opens the file unchanged and publishes `imageURL` to joiners, but its projector and cockpit do not yet show the picture. Route it through `Services.ImageCache` the way `GameView.BuildPicture` does; GIFs should animate

- [ ] 3.53 Clips reach the phones (Decision 060) — the Mac host publishes `pub.media` (https file link, or the clip re-encoded and written once to `live/{code}/media/{id}` under the 3 MB rules cap) and the cockpit says under Play whether the phones have it. Windows host: the same publish from `LiveNightHost.CurrentPub` (a C# `LiveRoom.Media` + `RoomMedia`; LibVLC or the raw file when already web-safe, size-checked), the once-per-room write on `LiveHostNet`, and the note line in the cockpit. Windows joiner (`JoinPlayerView`): the offer + LibVLC playback of the fetched node. ⏳

- [ ] 3.54 Projector adapts + full screen (macOS-DESIGN A8.9/A8.10, 2026-09-09) — the Mac projector was rebuilt: every element in the layout flow (hidden = no space), one 1280x720 design canvas scaled to the window (`ProjectorCanvas` = the Windows `Viewbox`), a fit-to-height middle column, the team standings as a wrapping chip STRIP, the join card as a horizontal band, the sponsor line in the flow; "Full screen" / "Full screen on <display>" in the cockpit Screen menu + double-click. Windows: audit `LiveCockpitView`'s projector for the same overlap classes with the per-element switches, port the strip/card/flow, and add `WindowState.FullScreen` (F11 + a cockpit button + per-display). Verify with the headless PNGs at 1280x720 and 1920x1080, all-on and all-off. ⏳
