...
| Living-room mode (phone-as-buzzer) | 🔮 controller | 🚧 controller | 🚧 host | 🔮 controller | Phase 1 Apple-native (Bonjour + room-code TLS-PSK) now **wired to a game mode** (Buzz Night, below): tvOS `BuzzerHost` (roster + per-seat scoring + wrong-buzz lockout/reopen + arbiter) and iOS `BuzzerJoinView` (`BuzzerClient` + big BUZZ button). Host lobby + in-game UI sim-verified; the two-device pairing/PSK handshake is hardware-only (Decision 030). Web/Android = Phase 2 web-room (Cloudflare DO). ROADMAP #4 |
| Trivia Night — TV-hosted (phone buzzers) | 🔮 | 🚧 controller | 🚧 host | 🔮 | Same name as the solo mode, hosted on the TV with phones (was "Buzz Night"). TV is stage + scoreboard, phones buzz, **the first to buzz then answers the question ON THEIR OWN PHONE**; the TV **celebrates each outcome** (who scored + points, a between-question leaderboard moment) and **every phone sees the same live story** (who buzzed / who got it / who missed + the scoreboard). A **per-question buzz clock** + all-locked-out detection mean the game **never hangs** — if everyone misses or nobody buzzes, the answer is revealed and it moves on. (the host streams the question + options to all phones via `question`/`answer`/`result` messages and judges the submitted answer); correct scores, wrong locks that seat out and re-opens to others; every Q ends on the Learn-the-fact reveal (GAME-MODES-RESEARCH D2). **Buzzable round types (audited, all display on TV + every phone):** General Knowledge (corpus MCQ — describe/cloze/Wikidata), **Picture Round** (the image streams to phones now, not just the TV), Which Came First? (2-option), Odd One Out. The input-required types (Closest Call slider, In Order, Match-Up, Name It text, Name As Many) are **excluded by design** — "buzz then tap one option" can't drive a slider / reorder / text box. The **room code stays on screen all game** for new players, but **rejoin is device-based**: each phone carries a stable id, the host keys seats by it, so a dropped phone resumes its **same seat + score regardless of name** — and a mid-game drop **auto-reconnects silently** (no tap, no re-entering the code; the code is also pre-filled next launch). tvOS `BuzzNightView_tvOS` (lobby → buzz/answer loop → standings) + iOS `BuzzerJoinView` (buzz + on-device answer buttons). Built + sim-verified to the host UI; the live buzz/answer handshake needs a 2-device hardware test (Ben). Web/Android via Phase 2 |
