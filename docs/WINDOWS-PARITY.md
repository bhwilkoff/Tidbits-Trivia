...
- [~] 3.11 Per-round wager/timer/speed — **final wager round** shipped end-to-end: a builder "make the last round a wager" toggle → LiveEvent.WagerFinalRound → host.WagerRoundIndex; the host publishes Pub.Wager on that round, AutoScore branches to ±stake (LiveScoring.WagerDelta, clamped ≥0, no speed bonus), and the join client shows a stake slider (0…your score, clamped in Send). Timer (3.23) + speed-tier (SpeedBonus) already done; per-round host-note pending. Wager-delta + event-flag + host-default tests
